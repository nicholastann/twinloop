...
const StatsBanner: React.FC = () => {
    ...
    return (
        <section className="bg-[#236a7c] text-white py-24 relative overflow-hidden">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16">
                        {stats.map((stat, index) => (
                            <div key={index} className="text-center group">
                                <div className="text-5xl md:text-7xl lg:text-8xl font-black mb-4 tracking-tighter text-white group-hover:scale-110 transition-transform duration-500 ease-out">{stat.value}</div>
                                <p className="text-lg md:text-2xl font-bold text-[#b8dce7] leading-snug max-w-[200px] mx-auto">{stat.label}</p>
                            </div>
                        ))}
                    </div>
                </ScrollFadeUp>
